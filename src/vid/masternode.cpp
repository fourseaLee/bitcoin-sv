...
void CMasternode::UpdateLastPaid(const CBlockIndex* pindex, int nMaxBlocksToScanBack)
{
    ...
    for (int i = 0; BlockReading && BlockReading->nHeight > nBlockLastPaid && i < nMaxBlocksToScanBack; i++) {
        if (mnpayments.mapMasternodeBlocks.count(BlockReading->nHeight) &&
            ...
            CAmount nMasternodePayment = 0;//= GetMasternodePayment(BlockReading->nHeight, block.vtx[0]->GetValueOut());
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    /*GetTransaction(vin.prevout.hash, tx2, Params().GetConsensus(), hashBlock, true);
    {
        ...
        if (mi != mapBlockIndex.end() && (*mi).second) {
            ...
            CBlockIndex* pConfIndex =  nullptr;//chainActive[pMNIndex->nHeight + Params().GetConsensus().nMasternodeMinimumConfirmations - 1]; // block where tx got nMasternodeMinimumConfirmations
