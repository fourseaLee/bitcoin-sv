...
CTxDestination CClueViewCache::FindClueAbleAddress(const CTxDestination& address, const uint32_t depth) const
{
    ...
    uint64_t maxSteps = 12;//(1 - pow(Params().ClueChildrenWidth(), depth)) / (1 - Params().ClueChildrenWidth());
    uint64_t nSteps = 0;
    ...
    while (queueDest.size() && maxSteps > 0) {
        ...
        if (ChildrenSize(root) < 12/*Params().ClueChildrenWidth()*/) {
            if (nSteps == 1)
                return root;
...
{
    bool fIsRoot = true; //IsClueRoot(address);
    if (!HaveClue(address)) {
        if (!fIsRoot) {
