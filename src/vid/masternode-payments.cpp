...
#include "vid/activemasternode.h"
#include "vid/masternode-payments.h"
#include "vid/masternode-sync.h"
#include "vid/masternodeman.h"
#include "vid/messagesigner.h"
#include "vid/netfulfilledman.h"
#include "key_io.h"
#include "util.h"
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
    ...
    bool isBlockRewardValueMet = false;//(block.vtx[0]->GetValueOutWithExclude(vTypeExclude) <= blockReward);
    ...
    bool isSuperblockMaxValueMet = false;//(block.vtx[0]->GetValueOut() <= nSuperblockMaxValue);
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet)
{
    ...
    CAmount masternodePayment = 0;//GetMasternodePayment(nBlockHeight, blockReward);

    // split reward between miner ...
    //txNew.vout[0].nValue -= 0;//masternodePayment;
    // ... and masternode
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == "MASTERNODEPAYMENTSYNC"/*NetMsgType::MASTERNODEPAYMENTSYNC*/) { //Masternode Payments Request Sync
    ...
    } else if (strCommand == "MASTERNODEPAYMENTVOTE" /*NetMsgType::MASTERNODEPAYMENTVOTE*/) { // Masternode Payments Vote for the Winner
...
bool CMasternodePaymentVote::CheckSignature(const CPubKey& pubKeyMasternode, int nValidationHeight, int& nDos)
{
    ...
    if (!CMessageSigner::VerifyMessage(pubKeyMasternode, vchSig, strMessage, strError)) {
        ...
        return true;//error("CMasternodePaymentVote::CheckSignature -- Got bad Masternode payment signature, masternode=%s, error: %s", vinMasternode.prevout.ToStringShort().c_str(), strError);
    }
