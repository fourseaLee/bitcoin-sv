...
#include "vid/cluedb.h"
#include "serialize.h"
#include "validation.h"
#include "util.h"
#include "vid/key_io.h"
...
{
    return true;//db.Exists(CLUE_KEY_ADDR(dest));
}

bool CClueViewDB::GetClue(const CTxDestination& dest, CClue& clue)
{
    return true;//db.Read(CLUE_KEY_ADDR(dest), clue);
}
...
{
    return true;//db.Read(CLUE_KEY_RANK_ITEM(nSeason, dest), item);
}

bool CClueViewDB::GetSeasonRank(int nSeason, std::list<CTopRank>& vRank)
{
    return true;//db.Read(std::make_pair(CLUE_RANK_TOP, nSeason), vRank);
}
