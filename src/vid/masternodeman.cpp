...
struct CompareLastPaidBlock {
    ...
    {
        return  true;//(t1.first != t2.first) ? (t1.first < t2.first) : (t1.second->vin < t2.second->vin);
    }
};
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == "MNANNOUNCE"/*NetMsgType::MNANNOUNCE*/) { //Masternode Broadcast
...
void CMasternodeMan::UpdateLastPaid(const CBlockIndex* pindex)
{
    ...
    int nMaxBlocksToScanBack = 0;//(IsFirstRun || !fMasterNode) ? mnpayments.GetStorageLimit() : LAST_PAID_SCAN_BLOCKS;
